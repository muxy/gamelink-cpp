...
void MGW_SDK_OnActionUsed(MGW_SDK Gateway, MGW_OnActionUsedCallback Callback, void* User)
{
	...
	SDK->OnActionUsed([=](const gateway::ActionUsed& ActionUsed)
	{
		MGW_ActionUsed Used;
		Used.TransactionID = ActionUsed.TransactionID.c_str();
		Used.ActionID = ActionUsed.ActionID.c_str();
		Used.Cost = ActionUsed.Cost;
		Used.UserID = ActionUsed.UserID.c_str();
		Used.Username = ActionUsed.Username.c_str();

		Callback(User, &Used);
	});
}

void MGW_SDK_AcceptAction(MGW_SDK Gateway, MGW_ActionUsed Coins, const char* Reason)
{
	gateway::SDK* SDK = static_cast<gateway::SDK*>(Gateway.SDK);

	gateway::ActionUsed tx;
	tx.ActionID = gateway::string(Coins.ActionID);
	...
	SDK->AcceptAction(tx, gateway::string(Reason));
}

void MGW_SDK_RefundAction(MGW_SDK Gateway, MGW_ActionUsed Coins, const char* Reason)
{
	gateway::SDK* SDK = static_cast<gateway::SDK*>(Gateway.SDK);
	...
	SDK->RefundAction(tx, gateway::string(Reason));
}
