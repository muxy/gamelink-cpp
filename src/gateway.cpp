...
namespace gateway
{
	...
	{
        return Base.AuthenticateWithGameIDAndRefreshToken(this->ClientID, this->GameID, JWT, [&](const gamelink::schema::AuthenticateResponse Resp)
        {
            gateway::AuthenticateResponse Auth(Resp.data.jwt, Resp.data.refresh, Resp.data.twitch_name, gamelink::FirstError(Resp) != NULL);
            Callback(Auth);
        });
    ...
    {
        return gamelink::ProjectionWebsocketConnectionURL(this->ClientID, gamelink::ConnectionStage::Sandbox, "c++", 0, 0, 1);
    }

    string SDK::GetProductionURL() const
    {
        return gamelink::ProjectionWebsocketConnectionURL(this->ClientID, gamelink::ConnectionStage::Production, "c++", 0, 0, 1);
    }
